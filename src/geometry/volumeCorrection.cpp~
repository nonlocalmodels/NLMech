...
geometry::VolumeCorrection::VolumeCorrection(data::DataManager *dataManager) {
	...
	correctVolume(dataManager->getModelDeckP()->d_horizon,
			dataManager->getMeshP()->getMeshSize(), dataManager->getNeighborP(),
...
void geometry::VolumeCorrection::correctVolume(const double &horizon,
		const double &dx, geometry::Neighbor *neighbors,
	...
	hpx::parallel::for_loop(hpx::parallel::execution::par, 0, nodes->size(),
			[&](boost::uint64_t i) {
				...
				(*d_volumeCorrection_p)[i] = std::vector<double>(
						neighbors->getNeighbors(i).size(), 1.);
				...
				for (auto j : neighbors->getNeighbors(i)) {
					...
					double r = dx * 0.5;
					if (util::compare::definitelyGreaterThan(X.length(),
							horizon - r))
						(*d_volumeCorrection_p)[i][k] = (horizon + r
								- X.length()) / dx;
