...
double material::pd::ConstInfluenceFn::getInfFn(const double &r) { return
d_a0; }
...
material::pd::LinearInfluenceFn::LinearInfluenceFn(const std::vector<double>
    &params, const size_t &dim)
...
material::pd::GaussianInfluenceFn::GaussianInfluenceFn(
  ...
  if (params.empty()) {
    ...
    if (dim == 1)
      ...
      d_alpha = (2.0/d_beta) * 1.0 / (d_beta - (d_beta + 1.) *
                std::exp(-1.0/d_beta));
  } else {
    d_alpha = params[0];
    d_beta = params[1];
...
double material::pd::GaussianInfluenceFn::getMoment(const size_t &i) {
  ...
  if (i==0) {
    ...
    return 0.5 * d_alpha * d_beta * sq1 * (0.5 * sq2 * std::erf(1./sq1)
          - (1./sq1) * std::exp(-1./d_beta));
  }
  else if (i==3) {
    // M3 = 0.5 * \alpha (\beta)^(2) * [1 - ((1/beta) + 1) * exp(-1/beta)]

    return 0.5 * d_alpha * d_beta * d_beta * (1. - (1. + 1./d_beta) *
    std::exp(-1./d_beta) );
  }
}