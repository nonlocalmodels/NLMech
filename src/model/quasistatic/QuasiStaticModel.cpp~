...
template<class T> model::QuasiStaticModel<T>::QuasiStaticModel(inp::Input *deck) :
...
template<class T> model::QuasiStaticModel<T>::~QuasiStaticModel() {
...
template<class T> void model::QuasiStaticModel<T>::initHObjects() {
	...
	std::cout << d_name << ": Initializing displacement loading object."
			<< std::endl;
	d_dataManager_p->setDisplacementLoadingP(
			new loading::ULoading(d_input_p->getLoadingDeck(),
					d_dataManager_p->getMeshP()));
	std::cout << d_name << ": Initializing force loading object." << std::endl;
	d_dataManager_p->setForceLoadingP(
			new loading::FLoading(d_input_p->getLoadingDeck(),
					d_dataManager_p->getMeshP()));
	...
	d_writer_p = new rw::writer::Writer(
			d_dataManager_p->getOutputDeckP()->d_outFormat,
	...
	if (d_dataManager_p->getOutputDeckP()->isTagInOutput("Stress_Tensor")){
		...
		d_dataManager_p->setStressTensorP(new std::vector<util::Matrix33>(d_nnodes));
	}

	if (d_dataManager_p->getOutputDeckP()->isTagInOutput("Strain_Tensor")){

		d_dataManager_p->setStrainTensorP(new std::vector<util::Matrix33>(d_nnodes));
	}
	

}

template<class T> void model::QuasiStaticModel<T>::computeForces(bool full) {
	...
	for (size_t i = 0; i < d_nnodes; i++) {
		...
		for (size_t j = 0; j < i_neighs.size(); j++) {
			...
			//m.lock();
			(*d_dataManager_p->getForceP())[j_id] -= res.first
					* (*d_dataManager_p->getMeshP()->getNodalVolumeP())[i];
			//m.unlock();
		...
		if (full){
			...
			{
				(*d_dataManager_p->getStrainEnergyP())[i] += (float)d_material_p->getBondEF(size_t(i),size_t(j_id)).second;
	...
	if(full){
			...
			{
				(*d_dataManager_p->getStrainTensorP())[i] = d_material_p->getStrain(size_t(i));
			...
			{
				(*d_dataManager_p->getStressTensorP())[i] = d_material_p->getStress(size_t(i));
...
template<class T> void model::QuasiStaticModel<T>::removeRow(
		util::Matrixij &matrix, size_t rowToRemove) {
...
template<class T> void model::QuasiStaticModel<T>::removeCol(
		util::Matrixij &matrix, size_t colToRemove) {
...
template<class T> void model::QuasiStaticModel<T>::removeRow(
		util::VectorXi &vector, size_t rowToRemove) {
...
template<class T> util::Matrixij model::QuasiStaticModel<T>::assembly_jacobian_matrix() {
	...
	util::parallel::copy<std::vector<util::Point3>>(
			*d_dataManager_p->getForceP(), backupForce);

	hpx::parallel::for_loop(hpx::parallel::execution::par, 0, d_nnodes,
			[&](boost::uint64_t i) {

				std::vector < size_t > *traversal_list =
						new std::vector<size_t>;

				traversal_list->push_back(i);

				auto i_neighs = d_dataManager_p->getNeighborP()->getNeighbors(
						i);
				for (auto j : i_neighs)

					traversal_list->push_back(j);

				for (auto j : *traversal_list) {

					for (size_t r = 0; r < dim; r++) {
						std::vector<util::Point3> eps_vector = std::vector
								< util::Point3 > (d_nnodes, util::Point3());
						...
						std::vector<util::Point3> *tmp = new std::vector<
								util::Point3>(d_nnodes, util::Point3());
						...
						for (size_t s = 0; s < dim; s++) {
							if (r == s)
								jacobian(i * dim + r, j * dim + s) = f_diff[r]
										/ (2. * eps);
	...
	util::parallel::copy<std::vector<util::Point3>>(backup,
...
template<class T> util::VectorXi model::QuasiStaticModel<T>::newton_step(
		util::VectorXi &res) {
	...
	util::VectorXi new_disp = util::VectorXi(
			d_nnodes * d_dataManager_p->getModelDeckP()->d_dim, 0.);
...
template<class T> void model::QuasiStaticModel<T>::solver() {
	...
	for (; d_n < d_input_p->getModelDeck()->d_Nt + 1; d_n++) {
		...
		d_dataManager_p->getForceLoadingP()->apply(d_time,
				d_dataManager_p->getBodyForceP(), d_dataManager_p->getMeshP());

		//Apply the displacement loading
		d_dataManager_p->getDisplacementLoadingP()->apply(d_time,
				d_dataManager_p->getDisplacementP(),
		...
		while (residual >= d_input_p->getSolverDeck()->d_tol
				and iteration < d_input_p->getSolverDeck()->d_maxIters) {
			...
			std::cout << "It: " << iteration << " Res: " << residual
					<< std::endl;
...
template<class T> util::VectorXi model::QuasiStaticModel<T>::computeResidual() {
	...
	for (size_t i = 0; i < d_nnodes; i++) {
		...
		for (auto bc : bcD) {
			for (auto d : bcN[k])
				...
				if (std::find(bcN[k].begin(), bcN[k].end(), i)
						!= bcN[k].end()) {
