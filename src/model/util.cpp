...
model::Output::Output(inp::Input *d_input_p,data::DataManager *d_dataManager_p, size_t d_n, double d_time){
...
auto writer = rw::writer::Writer(filename,d_input_p->getOutputDeck()->d_outFormat,
  ...
  if (d_input_p->getOutputDeck()->isTagInOutput(tag) &&
      d_policy_p->populateData("Model_d_eFB"))
      {
      double d_teFB = util::methods::add((*d_dataManager_p->getBBFractureEnergyP()));
