...
The open source code *NLMech* is an implementation of finite difference approximation of nonlocal models, \emph{e.g.}\ peridynamic. Peridynamic (PD) [@silling2007peridynamic,@silling2005meshfree] is a nonlocal formulation of classical continuum mechanics that is particularly robust with mechanical deformations involving crack and damage. The model seamlessly handles the two regime of deformation: elastic and smooth deformation and fracture. The constitutive law describing the material behavior is very simple to consemtualize and implement. Particularly, in numerical implementation no special care for modeling of cracks is required. Successful comparison of PD against a variety of experiments have been done [@diehl2019review]. 

Unlike classical continuum mechanics, where the internal forces in material is given by the stress at the material point, in PD, the internal force at given material point is the result of pairwise forces with the neighboring points. Ie the force is given by integral of pairwise force density between the given point and another point in the neighborhood. PD is typically divided in two classes of models: bond-based model and state-based models. In bond-based models, the two material points interact via a pairwise force law and the forces between the material points do not depend on the deformation state of surrounding points. In contrast, state-based models also describe the interaction between two material points but now the volumetric deformation in the neighborhood of two points play a role. The governing equation of motion for bond-based PD [@silling2005meshfree] reads as
